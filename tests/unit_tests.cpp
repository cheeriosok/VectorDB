...
TEST(ZSetTest, AddTest) {
    ...
    auto result1 = zset.add("Alice", 10.5).get();
    EXPECT_TRUE(result1);
    
    auto result2 = zset.add("Alice", 15.0).get(); // Updating score
    EXPECT_FALSE(result2);
    
    ZNode* node = zset.lookup("Alice");
    ASSERT_NE(node, nullptr);
    EXPECT_EQ(node->get_value(), 15.0);
}

TEST(ZSetTest, LookupTest) {
    ZSet zset;
    zset.add("Bob", 5.5).get();
    
    ZNode* node = zset.lookup("Bob");
    ASSERT_NE(node, nullptr);
    EXPECT_EQ(node->get_value(), 5.5);
    
    EXPECT_EQ(zset.lookup("NonExistent"), nullptr);
}
